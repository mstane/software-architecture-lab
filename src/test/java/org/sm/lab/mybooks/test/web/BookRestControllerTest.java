...
public class BookRestControllerTest extends BaseRestControllerTest {
    ...
    public void search() throws Exception {
        loadBooks(12);
        mockMvc.perform(get(RESOURCE_PATH + "?search=Deimos&pageNumber=2&pageSize=5")).andExpect(status().isOk())
                .andExpect(content().contentType(jsonContentType))
                ...
                .andExpect(jsonPath("$.content[0].shortContent", is("Author: " + "Tuth Jeggregh K'gassen10"
                        + "; Review: "
                        + "The book is fictional, it doesn't exist and the data are used as placeholder names.10")));
    ...
    public void create() throws Exception {
        ...
        mockMvc.perform(post(RESOURCE_PATH).with(csrf().asHeader()).contentType(jsonContentType).content(bookJson))
                .andExpect(status().isOk()).andExpect(content().contentType(jsonContentType))
                .andExpect(jsonPath("$.id", is(1))).andExpect(jsonPath("$.title", is(book.getTitle())));
    ...
    public void update() throws Exception {
        ...
        mockMvc.perform(put(RESOURCE_PATH + book.getId()).with(csrf().asHeader()).contentType(jsonContentType)
                .content(bookJson)).andExpect(status().isOk()).andExpect(content().contentType(jsonContentType))
                .andExpect(jsonPath("$.id", is(book.getId().intValue())))
    ...
    public void deleteOne() throws Exception {
        ...
        mockMvc.perform(delete(RESOURCE_PATH + book.getId()).with(csrf().asHeader()))
