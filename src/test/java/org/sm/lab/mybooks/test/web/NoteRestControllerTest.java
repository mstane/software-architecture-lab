...
public class NoteRestControllerTest extends BaseRestControllerTest {
  ...
  public void update() throws Exception {
    ...
    mockMvc.perform(put(RESOURCE_PATH + note.getId() + "?bookId=" + note.getBook().getId()).with(csrf().asHeader()).contentType(contentType).content(noteJson))
        .andDo(print())
  ...
  public void deleteNote() throws Exception {
    Note note = loadOneNote();
    mockMvc.perform(
        delete(RESOURCE_PATH + note.getId()).with(csrf().asHeader())
        )
