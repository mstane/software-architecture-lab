...
public class NoteRestControllerTest extends BaseRestControllerTest {
    ...
    public void getOne() throws Exception {
        ...
        mockMvc.perform(get(RESOURCE_PATH + note.getId())).andExpect(status().isOk())
    ...
    public void create() throws Exception {
        ...
        mockMvc.perform(post(RESOURCE_PATH + "?bookId=" + note.getBook().getId()).with(csrf().asHeader())
                .contentType(jsonContentType).content(noteJson)).andExpect(status().isOk())
                .andExpect(content().contentType(jsonContentType)).andExpect(jsonPath("$.id", is(1)))
                .andExpect(jsonPath("$.title", is(note.getTitle())));
    }
    ...
    public void update() throws Exception {
        ...
        mockMvc.perform(put(RESOURCE_PATH + note.getId() + "?bookId=" + note.getBook().getId()).with(csrf().asHeader())
                .contentType(jsonContentType).content(noteJson)).andDo(print()).andExpect(status().isOk())
                .andExpect(content().contentType(jsonContentType))
                .andExpect(jsonPath("$.id", is(note.getId().intValue())))
    ...
    public void deleteOne() throws Exception {
        ...
        mockMvc.perform(delete(RESOURCE_PATH + note.getId()).with(csrf().asHeader())).andExpect(status().isOk());
